...
export function LanguageSwitcher({ variant = 'dark', position = 'bottom' }: LanguageSwitcherProps) {
  ...
  const handleLocaleChange = (newLocale: Locale) => {
    ...
    const segments = pathname.split('/').filter(Boolean);
    const hasLocaleInPath = segments.length > 0 && locales.includes(segments[0] as Locale);
            ...
            {locales.map((locale) => (
              <button
                ...
                className={cn(
                  ...
                  currentLocale === locale ? "text-black font-medium" : "text-gray-700"
                )}
              >
                {localeNames[locale]}
              </button>
