...
export async function GET(
  ...
  try {
    // Extract portion after /products/ and strip query + trailing slashes
    const after = (req.url?.split("/products/")[1] || "").split("?")[0] || "";
    ...
    if (!fs.existsSync(fullPath)) {
      return res.status(404).json({ message: "File not found", path: relPath });
    }
